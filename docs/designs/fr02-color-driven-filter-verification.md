...
| 设计     | [重设计 V1.4 与高保真原型](./fr02-color-driven-filter-redesign.md) |
...
3. 纯色背景、显式文字、实际可见的 formatter Cell Bar、跨列/整行、保存/临时指标、
   字符串、百分比和时间对比派生列均进入相同颜色求值链路。
   原生规则顺序、覆盖关系、分页分母和 Cell Bar 范围不因筛选重新计算。
   V1.3 明确排除默认正负 Cell Bar 的筛选颜色，仅保留其绘制，见第 7 节。
...
## 7. V1.3：默认 Cell Bar 与色块提示修正（2026-08-29）

代码提交：`3daa5173f5`（默认柱条筛选范围）、`fbbb1653cb`（移除颜色名 Tooltip）。
设计和验收记录另行提交，不混入上述代码提交。

### 7.1 范围与根因

1. 原实现把默认正负柱条的红/绿一并加入 `colors` 和 `catalog`。修正仅移除
   前后端默认柱条的颜色族赋值，不改变 RGB、透明度、宽度、偏移或数值范围。
   显式 formatter `CELL_BAR` 的命中结果继续参与筛选；被背景遮盖、关闭、
   零宽、HTML 未渲染的柱条仍不计入。与默认柱条共存的显式文字/背景照常筛选。
2. 原色块 Tooltip 由 `isMenuItemFocused || isTooltipOpen` 控制；鼠标离开后
   菜单项仍可保持焦点，导致文字提示滞留。删除色块颜色名 Tooltip 及其焦点
   监听，而非强制取消用户焦点。保留色块、隐藏的辅助技术名称、
   `menuitemcheckbox`、`aria-checked` 和点击选择/取消能力。
   Formatter 渐变原因、Summary Tooltip 与数据单元格数值 `title` 不受影响。
3. 新增内部 `TABLE_COLOR_STYLE_REVISION` 到快照 generation。旧颜色索引不会
   被新语义复用；旧 token 返回 409 并要求重载，无 token 的新请求重建。
   请求协议仍为 `version=2`，不增加配置或数据库迁移，不清空共享 Redis。

本次不修改查询 SQL、ClickHouse/MySQL/PostgreSQL dialect、驱动或分页大小；
不更改任何保存的 Chart/Dataset，也不修复原生染色算法的其他既有行为。

### 7.2 回归证据

| 验证层次             | 本轮实际结果                                                         | 重点断言                                                            |
| -------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------- |
| 先失败后修正         | 颜色求值两端各 11 项失败；三色 Tooltip 3 项失败；旧快照版本 2 项失败 | 修复前能复现目标问题，不以本来已通过的测试代替复现                  |
| 后端联合单测         | 14 个文件，416 passed，4.89 秒                                       | 默认/显式柱条、可见性、缓存版本、权限、分页、汇总与导出             |
| 前端联合测试         | 25 suites，549 passed，74.629 秒                                     | 颜色求值、三色 hover/focus/移开/点击/重新打开、请求状态与兼容       |
| 独立复查             | TableChart + resolver 使用 `--no-cache`，134 passed；未发现 P1/P2    | 不是旧 Jest 缓存导致伪通过；显示和其他 Tooltip 未改                 |
| TypeScript           | 根 `tsc --noEmit` 通过                                               | 没有新增类型错误                                                    |
| 聚焦真实 HTTP        | 6 个业务场景 + cleanup 共 7 项，143 次请求，4.20 秒                  | 两种分页下默认柱条不入索引、显式柱条可筛选、混合样式与 XLSX         |
| 完整真实 HTTP        | 28 项，283 次请求，8.30 秒                                           | 额外回归百分比、时间对比、容量、错误、当前视图导出及跨 worker Redis |
| 聚焦真实 test-client | 7 项，143 次请求，25 次源 SELECT，3.73 秒                            | 初建后筛选/翻页/导出源 SELECT 增量为 0；不是 mock 数据              |

HTTP 场景每种分页均使用 180 行带正负、阈值边界的真实查询结果：

- 无 formatter 命中：默认柱条仍存在；各色选择为 0 行，清除后恢复 180 行；
  XLSX 保留 180 行的原柱条样式。普通无 formatter 查询保持可用，不激活颜色筛选。
- 显式黄色 Cell Bar：黄色匹配 125 行；默认绿色/红色不扩大结果；XLSX 也是 125 行。
- 黄色文字 + 红色背景 + 默认柱条：黄 125 行、红 2 行、合并 127 行；
  默认绿色不计入；值、顺序及柱条范围不因选择/清除重新计算。

聚焦 HTTP、完整 HTTP、聚焦 test-client 分别清理 8、24、8 个各自创建的
临时草稿；各次保存 Chart 变更计数均为 0。数量按独立执行记录，不累计冒充更多用例。

本次改动的定向 `pre-commit --files`、已暂存前端改动门禁、直接 Pylint 和
`git diff --check` 通过；Python 3.10 语法检查覆盖 5 个 Python 文件。
两份文档通过 Prettier，15 个本地链接目标存在，4 个 Mermaid 图块内容未改变。
上述 Pylint 分支比较沿用 `CI=1 GITHUB_BASE_REF=dev/6.1`，不改写第 6 节的
默认全仓门禁和真实 Python 3.10 运行时限制。

聚焦复跑（先启动本机服务，并设置本地 `SUPERSET_CONFIG_PATH`）：

```bash
venv/bin/python scripts/tests/validate_table_color_filters.py --transport http --cell-bars-only
venv/bin/python scripts/tests/validate_table_color_filters.py --transport test-client --cell-bars-only
```

### 7.3 实际浏览器与发布注意

- 已重启 Flask 后端，`8088/health` 与 `9001/health` 均为 200；使用当前
  前端开发服务和真实 Table Explore，不使用静态原型代替验收。
- 菜单项悬浮并移开鼠标后，焦点仍在 `menuitemcheckbox`，可见 Tooltip 为 0。
  点击绿色正常从 97 行筛为 24 行；重新打开菜单可见选中状态。
- 在未保存的 Explore 草稿中把已有绿色规则改为 `CELL_BAR`，Apply 后
  24 个匹配单元格真实绘制柱条，绿色选择仍得到 24 行。
- 将同一规则阈值临时改为不命中后，原匹配位置仍显示默认绿色柱条，但菜单
  只剩黄、红，不包含绿色。随后还原阈值和格式对象，不 Save 业务图表。
- 默认 `order_count` 柱条仍正常绘制；原菜单颜色名不再悬浮显示。

代码按“默认柱条筛选范围”和“颜色名 Tooltip”两个问题分别提交，本文与设计
文档另行提交。部署需前后端同步更新；持有旧快照的页面需重载。没有改发布开关
默认值，也不强推已分叉的远端。上文列出的生产性能、多数据库实连及完整可访问性
验收债务仍然存在，本轮定向通过不等同全仓或生产验收通过。

## 8. V1.4：色块旁显示完整基准行数（2026-08-29）

代码提交：`e8c7af7d0b`。设计和本节验收记录另行提交。

### 8.1 实现范围与计数口径

- 菜单显示“色块 `(数量)`”，保留右侧选中勾和整项点击；没有颜色名 Tooltip
  或原生 title。辅助技术名称包含颜色和“颜色过滤前的行数”。
- 新增可选响应字段 `table_color_metadata.color_counts`。每次从完整冻结
  `snapshot["styles"]` 的最终 `colors` 集合统计，而非当前页、导出投影或
  已经筛过的行。计数不随同列/跨列颜色选择、翻页或前端搜索变化。
- 同行同列同色只计一次；红底绿字可分别计入两色。默认 Cell Bar 不计入，
  显式 formatter Cell Bar 仍按既有可见性规则计入。
- 原查询、Native/Cross Filter、规则或强制刷新产生新基准时重新统计。
  旧快照无须增加持久化字段或重建；旧响应没有数量时只显示色块，不伪造零值。
  真实零值可显示 `(0)`，不支持列、不可用或保留旧结果的失败状态不显示数字。
- 生产改动仅涉及 Table 菜单/响应类型、快照计数函数和响应组装；没有新增 SQL、
  染色算法、配置项、数据库迁移或请求状态机制，不扩大默认 1,000 行边界。

### 8.2 本轮回归结果

| 层次                   | 实际结果                                              | 重点断言                                                                 |
| ---------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------ |
| 先失败后实现           | 前端 3 项计数测试失败；后端两种分页缺少 color_counts  | 新用例能暴露原实现缺口                                                   |
| 后端联合单测           | 14 个文件，436 passed，5.23 秒                        | 去重/跨色、两种分页、空集、规则/查询变化、旧缓存、零额外 SQL、导出       |
| 前端联合测试           | 25 suites，559 passed，91.276 秒                      | 数值显示/点击、0/缺省/非法值、失败/不支持、基准更新、无 Tooltip 和旧链路 |
| 最终 TableChart 无缓存 | `--no-cache`，105 passed，18.456 秒                   | 最终格式化后代码，无旧 Jest 缓存依赖                                     |
| TypeScript             | 根 `tsc --noEmit` 通过                                | 前端项目类型检查                                                         |
| 完整真实 HTTP          | 28 项，291 次请求，10.19 秒                           | 运行中 Flask、ClickHouse 21.3、Redis；每次 ready 响应计数一致            |
| 完整真实 test-client   | 27 项，288 次请求，80 次测试源 SELECT，8.78 秒        | 初建后颜色切换、分页、导出和额外计数校验的源 SELECT 增量为 0             |
| 跨 worker              | 独立 app 从 HTTP worker 快照读取，125 行，源 SELECT=0 | 完整基准计数和行/样式可跨进程复用                                        |
| 代码审查与定向门禁     | 9 个代码/测试文件 pre-commit 通过；直接 Pylint 10/10  | 复审未发现需修改问题；检查范围未扩展到无关代码                           |

真实原始记录用例额外叠加同色背景和文字，以独立条件计算预期数量，验证不会
把一行计成两行；Cell Bar 场景根据独立核对过的最终样式比对各色计数。选色、
清除、跨列、分页、当前视图/XLSX 导出后的查询及跨 worker 读取均断言同一
snapshot ID 的计数不变，不仅判断 HTTP 200。完整 HTTP / test-client 分别
清理 24 / 23 个测试专属临时草稿，保存 Chart 变更数均为 0。

上述测试数量为各自单次完整执行，不累计重跑。Python 3.10 语法检查覆盖本轮
6 个 Python 文件，通过；仍未在真正 Python 3.10 解释器下运行。定向门禁沿用
`CI=1 GITHUB_BASE_REF=dev/6.1`，不删除 checker、不修改第 6 节全仓门禁结论。
两份文档通过定向 pre-commit、Prettier 与 `git diff --check`；13 个去重后的
本地链接目标存在，4 个 Mermaid 图块内容未改变。

### 8.3 实际浏览器核验

已重启 Flask 加载计数实现，前端开发服务加载最终代码；使用真实 Explore，
不是设计原型。保存 Chart 配置未改动，客户端模式在未保存草稿中验证。

| 操作                               | 实际结果                                                        |
| ---------------------------------- | --------------------------------------------------------------- |
| 97 行基准，打开 gross_revenue 菜单 | 绿色 `(24)`、黄色 `(48)`、红色 `(25)`                           |
| 服务端点绿，再点黄                 | 24 → 72 行；选中勾正常，三色数量不变                            |
| 72 行结果切换第二页                | 数量仍为 24/48/25，不随本页数据变化                             |
| 叠加 quantity 绿色                 | quantity 菜单基准数 45；最终交集 30 行，收入三色数量仍不变      |
| 客户端分页点绿                     | 顶部 `24 rows`、`Search 24 records` 一致                        |
| 客户端搜索无匹配再打开菜单         | 表内 `No matching records found`；菜单仍为 24/48/25，清空可恢复 |
| 悬浮数量、移开、重新打开           | 无可见颜色名 Tooltip 或 title；色块/数量可选择，辅助名称含数量  |

沿用既有快照 TTL 和强制刷新入口；不更改失效恢复协议。仍保留第 6 节的生产
性能、多数据库实连和完整可访问性等验证债务。代码和文档分别提交，未推送或
覆盖分叉的远端历史；本地凭据及未跟踪目录未纳入提交。
