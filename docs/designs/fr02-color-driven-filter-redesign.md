...
不能只因配置 R 大于 50k 就拒绝实际只有 97 行的查询；用原 count 或 K+1 探测
...
| 配置建议                          | 初值                                    | 超限处理                                           |
| --------------------------------- | --------------------------------------- | -------------------------------------------------- |
| TABLE_ALERT_FILTER_MAX_ROWS       | 50,000 个图表结果行                     | 明确拒绝颜色快照，保留普通表；不把更大结果截成 50k |
...
不得悄悄变成“只筛当前页”。新增预算及首次加载成本须在实现前评审确认。
...
| PERF-01   | 10k/20k/50k，冷/热缓存，首次构建/重复选择                       | 记录 SQL 次数、时延、RSS；热选择/翻页不重跑基础查询                    |
