...
| 属性        | 内容                                                                                                        |
| ----------- | ----------------------------------------------------------------------------------------------------------- |
| 版本 / 日期 | V1.4 / 2026-08-29；色块菜单增加完整基准行数，沿用默认 Cell Bar 排除与无颜色名 Tooltip                       |
| 状态        | Implemented locally；本机实现与验收见[实施记录](./fr02-color-driven-filter-verification.md)，未部署生产     |
...
6. 先确定每个单元格**最终实际生效**的染色，再筛行。被覆盖的规则、不可见的
   Cell Bar、自动黑白可读文字都不能造成误选。默认正负 Cell Bar 仅用于显示，
   不作为筛选颜色来源；formatter 明确配置且实际可见的 Cell Bar 正常参与筛选。
...
默认 `Show cell bars` / 正负色配置不属于 formatter 染色规则，不提供筛选颜色。
即使同列有其他规则开启 `Enable alert filter`，也不能把默认柱条的红/绿加入
目录。显式 `Formatting object = Cell Bar` 规则沿用上述开关语义：命中且
实际可见时参与最终颜色集合，不要求同列每条染色规则都勾选开关。
...
| 色块文案            | 色块右侧显示灰色 `(数量)`，保留选中勾；不显示颜色名 Tooltip 或原生 title；保留辅助技术名称和 aria-checked |
...
色块右侧的 `(数量)` 表示**该列完整冻结基准中，最终包含该颜色的行数**：

- 先完成 Native/Cross Filter 和原查询后加工，再冻结基准；计数在所有颜色选择、
  分页及前端搜索之前。同列/跨列选色、翻页、前端搜索均不改变这个数字。
- 同一行、同一列、同一颜色只计一次；多条同色规则或同色背景/文字不重复计数。
  红底绿字可以同时计入红、绿，因此三色数量相加不一定等于总行数。
- 计数只读取已冻结 `styles[].colors`，与实际筛选使用同一颜色集合；默认
  Cell Bar 不计入，显式 formatter Cell Bar 按原可见性规则计入。
- Native/Cross Filter、查询、规则或刷新产生新基准后，随新结果一起更新计数。
  数字不是“叠加其他颜色选择以后，点击此色还能留下多少行”的动态预估。
- 旧响应未提供计数时只显示色块，不补假 `(0)`；真实零值显示 `(0)`，例如
  新基准中已选颜色不再出现。不支持列、快照不可用或保留旧结果的请求失败状态不显示数字。
- 使用既有 `Space` / `Typography.Text`，数值和色块均可点击选择；辅助名称说明
  “颜色过滤前的行数”，不重新引入颜色名悬浮提示。原型中已有的基准计数沿用此口径。
...
| 默认 Cell Bar      | 保留正负颜色、透明度、宽度和偏移，但不计入筛选颜色；同列启用其他规则也不改变该边界 |
| formatter Cell Bar | 只有显式规则命中、真实渲染且宽度大于零才计入；关闭、非数值、被背景抑制均不计入     |
...
设 `C(r,c)` 是行 `r` 在目标列 `c` 的最终可见、允许筛选来源的三色集合
（排除默认 Cell Bar），`S(c)` 是用户选择：
...
```json
{
  ...
  "color_counts": { "gross_profit": { "GREEN": 5, "YELLOW": 6, "RED": 5 } },
...
`color_counts` 为按结果列 key、颜色索引的非负整数行数；服务端对完整快照中的
可见列补全三色零值。普通页面、选色/翻页响应与导出元数据始终从完整
`snapshot["styles"]` 派生，不能从本页或导出投影计数。前端类型将其设为可选，
兼容旧响应缺省；非法负数、小数或非数值不展示。该字段不是新的请求参数。

计数不再执行 SQL，也不重算染色：响应阶段线性遍历已经存在的颜色集合，仅增加
每列三个整数。旧快照可直接派生计数，无需重建、清空缓存或提升样式版本；
不新增持久化缓存字段、配置项或迁移，默认 1,000 行和其他资源限制保持不变。
...
- token 不是权限凭证。创建和每次读取均重新校验用户/Guest、Dataset、Dashboard、
  ...
  原页长、主题、算法版本。
- 内部 `TABLE_COLOR_STYLE_REVISION` 参与 generation，独立于请求协议 `version=2`。
  V1.3 调整默认柱条的颜色索引来源后不复用旧快照；携带旧引用返回 409
  `TABLE_COLOR_FILTER_CONTEXT_CHANGED`，提示重载，不重跑 SQL 后伪装成旧快照命中。
  重载且省略旧引用时构建新快照；不清空其他查询的共享缓存。
...
| RULE-05    | formatter Cell Bar 开/关、零宽度、负值、背景覆盖、HTML          | 只索引显式规则命中且实际可见 bar，不因“命中过规则”误选                      |
| RULE-06    | 正向/反向时间对比，增/减/持平/缺失                              | 颜色而非业务意义决定行；灰色不产生新色块                                    |
| RULE-07    | 默认正负 bars、未命中规则、文字与默认 bar 共存、历史蓝色        | 默认 bar 不进入目录/行颜色；保留完整绘制；显式 formatter bar 正常筛选       |
| UI-TOOLTIP | 三色色块 hover/focus、移开、点击关闭、重新打开                  | 无颜色名提示残留；保留辅助技术名称、选中态、颜色选择与取消                  |
| COUNT-01   | client/server、同列三色、跨列选择、翻页、前端搜索               | 数量来自所有颜色选择和分页前的完整基准，不变成当前页或交集数量              |
| COUNT-02   | 同色背景/文字、重复规则、红底绿字、默认/显式 Cell Bar           | 同行同列同色去重，跨色可重叠；默认柱条不计入，显式柱条正常计入              |
| COUNT-03   | 旧响应缺省、真实零值、非法数值、不支持列、保留旧结果的请求失败  | 缺省/异常不补假零；真实零可见；不可用或失败状态不展示旧数字                 |
| COUNT-04   | 新基准、旧缓存、清除颜色、Current View 导出及跨 worker 读取     | 新基准重新计数；旧缓存直接派生；同一快照计数稳定，额外源 SELECT 为 0        |
...
| CACHE-02   | 染色/索引内部版本变化、旧引用、新请求、再次命中                 | 旧引用 409 且不执行 SQL；无旧引用重建新目录；后续复用新快照                 |
...
### 12.3 实施归档（2026-08-29）

功能和测试已在 `a04e266124` 独立提交，保留前面的设计阶段历史记录，不将原型
验证重新解释为生产测试。完整证据、复跑命令、部署条件及未验证范围见
[FR-02 颜色驱动筛选实施与本机验收记录](./fr02-color-driven-filter-verification.md)。

首次实现定向验证：前端 25 suites / 533 tests、后端 402 tests 通过；运行中的本机
ClickHouse 21.3、Redis、Superset HTTP 和实际页面已验证主要查询/筛选/导出链路。
默认 1,000 行预算、渐变明确提示、当前视图投影、跨列选择集合比较和隐藏列清理
均已实现。尚未完成生产性能、多数据库实连、完整可访问性等验收，不宣称全仓
默认门禁或生产部署已通过。本次未推送，未覆盖远端分叉历史。

V1.3 补充默认 Cell Bar 排除与无颜色名 Tooltip 的实现和回归；保留 formatter
Cell Bar 的正常绘制和筛选，逐项证据见上述验收记录第 7 节。

V1.4 在色块旁增加完整基准行数，不新增 SQL 或扩大快照上限。联合验证为
前端 25 suites / 559 tests、后端 436 tests，真实 HTTP 全流程 28 项；
计数语义、兼容性及实际页面证据见上述验收记录第 8 节。
